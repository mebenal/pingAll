...
class Ping2
{
    ...
    /// </summary>
    public async static Task PingHost(IPAddress to, IPAddress from)
    {
        ...
        try
        {
            ...
            IPEndPoint ipEndPointTo = new IPEndPoint(to, 0);
            EndPoint EndPointTo = (ipEndPointTo);
            // Set the receiving endpoint to the client machine
            IPEndPoint ipEndPointFrom = new IPEndPoint(from, 0);
            ...
            if ((nBytes = socket.SendTo(sendbuf, PacketSize, 0, EndPointTo)) == SOCKET_ERROR)
            {
                Console.WriteLine("Socket Error cannot Send Packet");
            }
            WaitForData(socket, to);
            return;
            ...
            while (!recd)
            {
                ...
                else if (nBytes > 0)
                {
                    ...
                    //Console.WriteLine("Reply from " + EndPointTo.ToString() + " in "
                    //+ dwStop + "MS :Bytes Received" + nBytes);
                    recd = true;
                    //Console.WriteLine($"{to.ToString()} {ReceiveBuffer[20]}");
                    //PrintByteArray(ReceiveBuffer); ;
                    if (ReceiveBuffer[20] == 0)
                    {
                        Interlocked.Increment(ref Program._addrSuccess);
                    }
                    break;
                }
                if (timeout > 2000)
                {
                    //Console.WriteLine($"{to.ToString()} Time Out");
                    recd = true;
                }
            }
            //close the socket
            Interlocked.Increment(ref Program._addrPinged);
            socket.Close();
            return;
        }
    ...
    */
    ...
    public static void WaitForData(Socket m_socClient, IPAddress addr)
    { 
        AsyncCallback pfnCallBack = new AsyncCallback(OnDataReceived);
        // now start to listen for any data...
        CSocketPacket theSocPkt = new CSocketPacket();
        theSocPkt.thisSocket = m_socClient;
        theSocPkt.Address = addr;
        // now start to listen for any data...
        m_socClient.BeginReceive(theSocPkt.dataBuffer, 0, theSocPkt.dataBuffer.Length, SocketFlags.None, pfnCallBack, theSocPkt);
    }
    public static void OnDataReceived(IAsyncResult asyn)
    {
        try
        {
            ...
            Console.WriteLine($"{theSockId.Address} {theSockId.dataBuffer[20]}");
...
public class CSocketPacket
{
    ...
    public byte[] dataBuffer = new byte[128];
}